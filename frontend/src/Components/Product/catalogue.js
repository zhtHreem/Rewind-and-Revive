...
import { Box, Grid, Paper, Typography, Button, Checkbox,Link, FormControlLabel, useMediaQuery, Card, CardMedia, CardContent, FormControl, Select, InputLabel, MenuItem, IconButton } from "@mui/material";
...
const CataloguePage = () => {
    ...
    return filters.length > 0 ? filters : ['No filters applied']; // Return as an array
};

    useEffect(() => {
        setShowFilters(getAppliedFiltersSummary());
    }, [category, selectedSubcategories, priceRange, size]);

    const handleRemoveFilter = (filterToRemove) => {
    // Remove the filter from the showFilters array
    ...
    if (filterToRemove.startsWith('Category:')) {
        setCategory('');
    } else if (filterToRemove.startsWith('Subcategories:')) {
        const subcategoriesToRemove = filterToRemove
            .replace('Subcategories:', '')
            .split(', ')
            .map(subcat => subcat.toLowerCase());
        setSelectedSubcategories(prev =>
            prev.filter(subcat => !subcategoriesToRemove.includes(subcat.toLowerCase()))
        );
    ...
    return (
        ...
        <Box p={5}>
            ...
            {isMobile && (
                <Button variant="contained"  onClick={() => setFilterOption(!filterOption)} sx={{ mb: 2 }} endIcon={filterOption ? <ExpandMore /> : <KeyboardArrowDown />}>
                    Filters
                </Button>
            )}
            ...
            <Box sx={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row'}}>
                ...
                {(isMobile && filterOption) || !isMobile ? (
                    <Box sx={{ width: isMobile ? '100%' : '13%', p: 2, border: !isMobile ? '1px solid #ddd' : 'none', mb: isMobile ? 2 : 0 }}>
                        ...
                        {/* Category Filter */}
                        <Box sx={{ mb: 2 }}>
                            <Button  variant="subtitle1"  onClick={() => setCategory(category ? '' : 'category')}  sx={{ color: "black", width: "100%", borderBottom: "1px outset black", borderLeft: "1px outset black" }}  endIcon={<KeyboardArrowDown />} >
                                Category
                            </Button>
                            {category && (
                                <Box sx={{ pl: 2 }}>
                                    {uniqueCategories.map(cat => (
                                        <Box key={cat} sx={{ mb: 2 }}>
                                           <Button fullWidth onClick={() => setCategory(cat)}  sx={{ color: "black", width: "100%", borderBottom: "1px outset black", }} endIcon={<KeyboardArrowDown />} >
                                                        ...
                                                        <FormControlLabel
                                                            ...
                                                            control={
                                                                <Checkbox  checked={selectedSubcategories.includes(subcat)}
                                                                    onChange={() => { setSelectedSubcategories(prev =>   prev.includes(subcat)  ? prev.filter(item => item !== subcat)  : [...prev, subcat]);   }} />
                                                            }
                        ...
                        <Box sx={{ mb: 2 }}>
                             ...
                             <Box sx={{ pl: 2 }}>
                                        <FormControlLabel
                                             control={
                                                 <Checkbox checked={priceRange === '100'} onChange={() => setPriceRange(priceRange === '100' ? '' : '100')}  />
                                                 }
                                            label="Up to $100" />
                                       <FormControlLabel
                                              control={
                                                 <Checkbox checked={priceRange === '200'} onChange={() => setPriceRange(priceRange === '200' ? '' : '200')}  />
                                                 }
                                            label="Up to $200" />
                                       <FormControlLabel
                                              control={
                                                <Checkbox  checked={priceRange === '300'} onChange={() => setPriceRange(priceRange === '300' ? '' : '300')}  />
                                                }
                                            label="Up to $300"  />
                             </Box>
                        </Box>
                        ...
                        <Box sx={{ mb: 2 }}>
                            ...
                            <Box sx={{ pl: 2 }}>
                                 <FormControlLabel control={ <Checkbox  checked={size === 'S'} onChange={() => setSize(size === 'S' ? '' : 'S')}/> } label="Small"/>
                                 <FormControlLabel control={  <Checkbox checked={size === 'M'} onChange={() => setSize(size === 'M' ? '' : 'M')}/>   }  label="Medium"   />
                                 <FormControlLabel control={  <Checkbox checked={size === 'L'} onChange={() => setSize(size === 'L' ? '' : 'L')} /> } label="Large"  />
                            </Box>
                        </Box>
                ...
                <Box sx={{ width: isMobile ? '100%' : '75%', p: 2 }}>
                    ...
                    <Box display="flex"  justifyContent={"space-between"} >
                        <Typography variant="body2" >Showing 0 of 0 items</Typography>
                        <FormControl  sx={{minWidth: 150, maxWidth: 300 }}>
                            <InputLabel id="demo-simple-select-label">Choose</InputLabel>
                            <Select labelId="demo-simple-select-label" id="demo-simple-select" label="Age" >
                             <MenuItem value={10}>Popularity</MenuItem>
                             <MenuItem value={20}>High-to-Low price</MenuItem>
                             <MenuItem value={30}>Low-to-High price</MenuItem>
                             </Select>
                        </FormControl>
                    </Box>
